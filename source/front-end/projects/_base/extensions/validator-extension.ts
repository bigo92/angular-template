...
export class ValidatorExtension {
  ...
  static maxDate(max: Date, errorMessage?: string) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if(value.getFullYear() > max.getFullYear() ||
        (value.getFullYear() === max.getFullYear() && value.getMonth() > max.getMonth()) ||
        (value.getFullYear() === max.getFullYear() && value.getMonth() === max.getMonth() && value.getDate() > max.getDate())){
        return {
          error: errorMessage || `Giá trị ngày tối đa là ${max.getDate()+1}/${max.getMonth()}/${max.getFullYear()}`,
          maxDate: true
        }
      }
      return null;
  ...
  static number(errorMessage?: string) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      return NUMBER_REGEXP.test(control.value) ? null : { error: errorMessage || 'Không đúng định dạng số', number: true };
    };
  }
  ...
  static nullOrLength(arrLen: number[] ,errorMessage?: string) {
    return (control: AbstractControl) => {
      ...
      if (!arrLen.includes(lengthControl)) {
        return { error: errorMessage || `Phải chứa ${arrLen.join(' hoặc ')} ký tự`, space: true };
      }
      return null;
    };
  }
  ...
  static gteDateValidator(myForm: FormGroup, controlTarget: string, errorMessage: string) {
    return (control: FormControl) => {
      ...
      if (!control1 || !control2) {
        return null;
      } else if (control1.getFullYear() < control2.getFullYear() ||
                (control1.getFullYear() === control2.getFullYear() && control1.getMonth() < control2.getMonth()) ||
                (control1.getFullYear() === control2.getFullYear() && control1.getMonth() === control2.getMonth() && control1.getDate() < control2.getDate())) {
        return { error: errorMessage };
      }
      return null;
    };
